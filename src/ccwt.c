...
int ccwt_calculate(struct ccwt_data* ccwt, void* user_data, int(*callback)(struct ccwt_data*, void*)) {
    ...
    for(unsigned long y = 0; y < ccwt->height && !return_value; ++y) {
        ...
        return_value = callback(ccwt, user_data);
    }
    fftw_destroy_plan(input_plan);
